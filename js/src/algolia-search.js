$(document).ready(function() {
  ...
  if (a.applicationID && a.apiKey && a.indexName) {
    ...
    [instantsearch.widgets.searchBox({
      ...
      templates: {
        item: function(a) {
          return '<a href="' + (a.permalink ? a.permalink : CONFIG.root + a.path) + '" class="algolia-hit-item-link">' + a._highlightResult.title.value + "</a>"
        },
        empty: function(i) {
          return '<div id="algolia-hits-empty">' + a.labels.hits_empty.replace(/\$\{query}/, i.query) + "</div>"
        }
      },
      cssClasses: {
      ...
      templates: {
        body: function(i) {
          return a.labels.hits_stats.replace(/\$\{hits}/, i.nbHits).replace(/\$\{time}/, i.processingTimeMS) + '<span class="algolia-powered">  <img src="' + CONFIG.root + 'images/algolia_logo.svg" alt="Algolia" /></span><hr />'
        }
      }
    ...
    })].forEach(i.addWidget, i), i.start(), $(".popup-trigger").on("click", function(a) {
